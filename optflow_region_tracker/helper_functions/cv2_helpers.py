...
def canvas_to_BGR(canvas):
    ...
    """
    byte_string, wh = canvas.print_to_buffer()  #  Returns bytestring of RGBA and (width, height)
    img = np.frombuffer(byte_string, dtype=np.uint8, count=wh[0]*wh[1]*4).\
                reshape((wh[1], wh[0], 4))[:,:,:3][:,:,::-1]  #  cut off A channel, then reorder 3rd dimension from RGB to BGR
    return img

def label_w_background(frame0, text1='', text2=0, bg_col=(255,255,255), txt_col=(0,0,0),
                       display_str='{:9s} corner of region {:d}', origin=(0,0), txt_scale=.4):
    ...
    text_size, baseline = cv2.getTextSize(
            text,
            cv2.FONT_HERSHEY_COMPLEX, txt_scale, 1)
#    print(text_size)
    cv2.rectangle(frame0, origin, (origin[0]+text_size[0], origin[1]+text_size[1]),
            bg_col, -1)
    cv2.putText(frame0, text, (origin[0], origin[1]+text_size[1]), cv2.FONT_HERSHEY_COMPLEX, txt_scale, txt_col )
    return frame0

def draw_roi(initial_frame, topleft, bottomright, label='', box_col=(0,255,0), thickness=2,
             font=cv2.FONT_HERSHEY_COMPLEX, fontscale=.7, offset=(0,0)):
    ...
    cv2.putText(initial_frame, str(label),
                            (int(topleft[0]+offset[0]), int(bottomright[1]+offset[1]) ),
                            font, fontscale, box_col)
#%%
def region_select_callback(window_name, initial_frame, lefttops, rightbottoms,
str_seq=['top left','bot right'], box_col=(0, 255, 0),label_origin=(0,0)):
    ...
    def callback(event,x,y,flags,param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            if callback._seq:
                lefttops.append([x,y])
                cv2.imshow(window_name, label_w_background(
                                   initial_frame, str_seq[1], callback._num_regions,
                                   origin=label_origin )
                          )
                ...
                draw_roi(initial_frame, lefttops[-1], rightbottoms[-1], box_col=box_col,
                ...
                cv2.imshow(window_name,
                           label_w_background(initial_frame, str_seq[0], callback._num_regions,
                           origin=label_origin)
                           )
                callback._seq = not callback._seq
            ...
            if len(lefttops)>0:
                if len(lefttops)>len(rightbottoms):
                    ...
                    callback._seq = not callback._seq
                    cv2.imshow(window_name, label_w_background(
                                   initial_frame, str_seq[0], callback._num_regions,
                                   origin=label_origin)
                          )
                else:
                    callback._num_regions-=1
                    ...
                    #  cross it out
                    cv2.line(initial_frame, tuple(lefttops[-1]), tuple(rightbottoms[-1]), box_col, 2 )
                    cv2.line(initial_frame, (lefttops[-1][0], rightbottoms[-1][1]),
                                    (rightbottoms[-1][0], lefttops[-1][1]), box_col, 2 )
                    cv2.imshow(window_name,
                           label_w_background(initial_frame, str_seq[0], callback._num_regions,
                           origin=label_origin )
                           )
...
def selectROIs(winname, frame, box_col=(0,250,0), showCrosshair=True, label_origin=(0,0)):
    ...
    callback = region_select_callback(winname, frame, tls, brs, box_col=box_col, label_origin=label_origin)
    ...
    if len(brs)>len(tls):
        print('[WARNING] Selected final top left without selecting corresponding bottom right.')
        tls = tls[:-1]
...
def along_line_select_callback(window_name, initial_frame, startpoints, endpoints,
str_seq=['start point','end point'], line_col=(0, 255, 0),label_origin=(0,0)):
    """Returns a function which can be used as a callback of a cv2 window to
    ...
    def callback(event,x,y,flags,param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            if callback._seq:
                ...
                # Add the point:
                cv2.drawMarker(initial_frame,
                               tuple(startpoints[-1]),
                               line_col, selected_point_style, point_size, point_thick  )
                cv2.imshow(window_name, label_w_background(
                                   initial_frame, str_seq[1], callback._num_lines,
                                   origin=label_origin,
                ...
                cv2.drawMarker(initial_frame,
                               tuple(endpoints[-1]),
                               line_col, selected_point_style, point_size, point_thick )
                cv2.line(initial_frame, tuple(startpoints[-1]), tuple(endpoints[-1]),
                    line_col, 2)
                # Put a little label with the region number, too
                ...
                cv2.imshow(window_name,
                           label_w_background(initial_frame, str_seq[0], callback._num_lines,
                           origin=label_origin,
                           display_str=display_str)
                           )
                callback._seq = not callback._seq
            ...
            if len(startpoints)>0:
                ...
                cv2.drawMarker(initial_frame,
                               tuple(startpoints[-1]),
                               line_col, deleted_point_style, point_size, point_thick )
                if len(startpoints)>len(endpoints):
                    ...
                    callback._seq = not callback._seq
                    cv2.imshow(window_name, label_w_background(
                                   initial_frame, str_seq[0], callback._num_lines,
                                   origin=label_origin,
                                   display_str=display_str )
                          )
                else:
...
def _rois_along_line(startpoint, endpoint, wh, dist):
    ...
    """
    startpoint = np.array(startpoint); endpoint = np.array(endpoint)
    w,h = wh
    ...
    if len(center_points)==0:
        ...
        warnings.warn('Line was too short for even a single ROI; putting one in the midpoint')
...
#%%
def selectROIs_along_line(winname, frame0, line_col=(0,250,0), showCrosshair=True, label_origin=(0,0),
            ROIsize=(30,30), dist=5):
    ...
    frame = frame0.copy()
    # generate the callback function
    callback = along_line_select_callback(winname, frame, spts, epts, line_col=line_col,
                    label_origin=label_origin)
    # register it with the window
    cv2.setMouseCallback(winname,callback)
    cv2.imshow(winname, label_w_background(frame, 'start point', 0, origin=label_origin,
    ...
    if len(spts)>len(epts):
        print('[WARNING] Selected final start point without selecting corresponding end point.')
        epts = epts[:-1]
    ...
    for ii, roi in enumerate(ROIs):
        ...
        draw_roi(frame, (roi0[0], roi0[1]), (roi0[0]+roi0[2], roi0[1]+roi0[3] ), label=ii, box_col=line_col )
    cv2.imshow(winname, frame)
    _ = cv2.waitKey(1)
    return ROIs

#%% Region selection with circle
def circle_select_callback(window_name, initial_frame, centers, perimeter_points,
str_seq=['center','pt on perimeter'], line_col=(0, 255, 0),label_origin=(0,0)):
    """Returns a function which can be used as a callback of a cv2 window to
    ...
    def callback(event,x,y,flags,param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            if callback._seq:
                ...
                cv2.drawMarker(initial_frame,
                               tuple(centers[-1]),
                               line_col, selected_center_style, point_size, point_thick  )
                cv2.imshow(window_name, label_w_background(
                                   initial_frame, str_seq[1], callback._num_circles,
                                   origin=label_origin,
                                   display_str=display_str )
                          )
                callback._seq = not callback._seq
                ...
                cv2.drawMarker(initial_frame,
                               ...
                               line_col, selected_perimeter_style, point_size, point_thick )
                cv2.line(initial_frame, tuple(centers[-1]), tuple(perimeter_points[-1]),
                    line_col, 2)
                print('New points: {}, {}.'.format(centers[-1], perimeter_points[-1]))
                # Increment the region counter
                callback._num_circles += 1
                cv2.imshow(window_name,
                           label_w_background(initial_frame, str_seq[0], callback._num_circles,
                           origin=label_origin,
                           display_str=display_str)
                           )
                callback._seq = not callback._seq
            ...
            if len(centers)>0:
                ...
                if len(centers)>len(perimeter_points):
                    ...
                    callback._seq = not callback._seq
                    cv2.imshow(window_name, label_w_background(
                                   initial_frame, str_seq[0], callback._num_circles,
                                   origin=label_origin,
                                   display_str=display_str )
                          )
                else:
...
def _rois_on_circle(center, perimeter_point, wh, arc_dist):
    ...
    w, h = wh
    radius = np.sqrt( (perimeter_point[0]-center[0])**2 +
                        (perimeter_point[1]-center[1])**2 )
    theta0 = np.angle( perimeter_point[0] - center[0] +
                        1j*(perimeter_point[1] - center[1]) )
    center_points = [
...
def selectROIs_on_circle(winname, frame0, line_col=(0,250,0), showCrosshair=True, label_origin=(0,0),
            ROIsize=(30,30), num_rois=10, discard_outside=True):
    ...
    frame = frame0.copy()
    # generate the callback function
    callback = circle_select_callback(winname, frame, spts, epts, line_col=line_col,
                    label_origin=label_origin)
    # register it with the window
    cv2.setMouseCallback(winname,callback)
    cv2.imshow(winname, label_w_background(frame, 'center', 0, origin=label_origin,
    ...
    # Once we exited, make sure that we didn't exit without selecting the last bottom right corner
    if len(spts)>len(epts):
        print('[WARNING] Selected final center without selecting corresponding point on perimeter.')
        epts = epts[:-1]
    ...
    for center, perimeter_point in zip(spts, epts):
        ...
        if discard_outside:
            rois = [roi for roi in rois if not
...
def center_select_callback(window_name, initial_frame, centers,
    str_seq=['center'], line_col=(0, 255, 0),label_origin=(0,0)):
    """Returns a function which can be used as a callback of a cv2 window to
    select a circle along which ROIs are generated automatically: one in the
    center, and a specified number along the perimeter
    ...
    def callback(event,x,y,flags,param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            cv2.imshow(window_name,
                        label_w_background(initial_frame, str_seq[0], callback._num_centers,
                        origin=label_origin,
                        display_str=display_str)
                        )
        elif event == cv2.EVENT_RBUTTONDOWN:
            if len(centers)>0:
                ...
                centers.pop()
                cv2.imshow(window_name, label_w_background(
                                initial_frame, str_seq[0], callback._num_centers,
                                origin=label_origin,
                                display_str=display_str )
                          )
            else:
    ...
    return callback
#%%
def selectROIs_by_center(winname, frame0, line_col=(0,250,0), showCrosshair=True, label_origin=(0,0),
            ROIsize=(30,30), discard_outside=True):
    """A replacement for cv2's cv2.selectROIs().
    ...
    frame = frame0.copy()
    # generate the callback function
    callback = center_select_callback(winname, frame, spts, line_col=line_col,
                    label_origin=label_origin)
    # register it with the window
    cv2.setMouseCallback(winname,callback)
    cv2.imshow(winname, label_w_background(frame, 'center', 0, origin=label_origin,
    ...
    for center in spts:
        ...
        if discard_outside and (roi[0]<0 or roi[1]<0 or roi[0]+roi[2]>W or roi[1]+roi[3]>H):
            pass
        else:
            ROIs+=[roi]
    for ii, roi in enumerate(ROIs):
