...
from itertools import zip_longest  #  neeeded to transpose a list of possibly uneven lists
import numpy as np
...
def HD_4panel(left_top_left_top_w_h=(0,0,0,360,480,360)):
    ...
    vislight = ( slice(yvis, yvis+h, None),
                 slice(xvis, xvis+w, None))
    infra = ( slice(ynir, ynir+h, None),
                 slice(xnir, xnir+w, None))
    def transform(frame):
        return frame[vislight],\
...
def setup_input_stream(stream_type,skip_frames,frames_to_process,
            abs_fname=None,offset_ms=None,transform=None):
...
def select_rois_manual(vis0,display_str=""):
    ...
    selected_rois = cv2h.selectROIs(select_window_name, initial_frame, showCrosshair=True,
            label_origin=(100,0))
...
def select_rois_center(vis0, w, h):
    ...
    initial_frame = vis0.copy()
    selected_rois = cv2h.selectROIs_by_center(select_window_name, initial_frame,
                        label_origin=(100,0), ROIsize=(w,h))
    return selected_rois
...
def select_rois_along_line(vis0, w, h, dist):
    ...
    """
    initial_frame = vis0.copy()
    selected_rois = cv2h.selectROIs_along_line(select_window_name, initial_frame,
                        label_origin=(100,0), ROIsize=(w,h), dist=dist)
    return selected_rois
...
def select_rois_on_circle(vis0, w, h, num_rois):
    ...
    """
    initial_frame = vis0.copy()
    selected_rois = cv2h.selectROIs_on_circle(select_window_name, initial_frame,
                        label_origin=(100,0), ROIsize=(w,h), num_rois=num_rois)
    return selected_rois
...
def make_parser():
    parser = argparse.ArgumentParser(description='Load video and select ROIs. \
        ROIs are then tracked and their brightness is stored as a CSV file, along with the initial frame and (optionally) a video of the tracking.',
    ...
    videos.add_argument('--merged_video',type=str,help='Absolute path to the video containing BOTH, visible light and nir image, \
        as it would be generated by the Stryker system')
    videos.add_argument('--vis_nir_videos',type=str,nargs=2,help='Absolute paths to the videos containing visible light and nir image, \
        visible light first. This is for when two separate videos are generated')
    videos.add_argument('--separate_vids',action='store_true',help='If present, then two file dialogues open:\
        the first for the visible light video, the second for the nir video.')

    parser.add_argument('-s','--shortname', required=False,type=str,
                            help='Patient name/id. Used for the filenames',default='no_name')
    parser.add_argument('--start', type=float,
                            help='Time offset from start of video (seconds).', default=0.0)
    parser.add_argument('--end', type=float,
                            help='Time offset from start of video to end of tracking (seconds). \
                                If not given, tracking runs until stopped manually or video file ends.')
    parser.add_argument('--fps','--apparent_fps', type=float,
                            help='In some cases, the video file reports a different framerate from what \
                                is seen in Python, e.g. even though the reported framerate is 60FPS, \
                                reading 60 frames in Python actually corresponds to 2 seconds of video \
                                (instead of just one); for such cases, pass this "effective framerate"\
                                in this argument. In particular for the videos from the NOVADAQ system,\
                                pass "--fps 29.95".')
    parser.add_argument('--panel-locations', type=int, help=
        '''The locations of the panels for visible light and NIR respectively in the merged video.
        Specify them as
        ...
        --panel-locations 0 0 0 360 480 360''',
                        nargs=6, default=(0, 0, 0, 360, 480, 360))
    #
    parser.add_argument('--no-tracking', help='If given, no actual tracking is done, i.e. the ROIs do not move.',
                            action='store_true')

    #  storing results and accessing previous configurations
    parser.add_argument('--store_folder',type=str, help='Path where to store the resulting CSV file and the tracking vid, if applicable. \
        If not given, a file dialogue is opened.')
    parser.add_argument('--fill_value', help='Fill value to be used for intensities of regions that are lost already when writing to CSV. \
                            If not given then the cells are empty.')
    parser.add_argument('--no-tracking-vid', help='If given, does not store a video of the tracking.',
                            action='store_true')
    parser.add_argument('--no-plot-stored', help='If given, does not stort a plot of the intensities.',
                            action='store_true')
    #  Show the frames as we're tracking?
    parser.add_argument('--show-tracking', help='If given, shows the frames that are being processed.',
                            action='store_true')
    parser.add_argument('--tracking-max-height',type=int,help='If the video stacking visible and NIR light frames is \
        taller than that (in pixels), it will be resized', default=1080, dest='maxheight')
    parser.add_argument('--tracking-max-width',type=int,help='If the video stacking visible and NIR light frames is \
        taller than that (in pixels), it will be resized', default=1920//3, dest='maxwidth')
    parser.add_argument('--hide-intensities', help='If given, does not show the live intensities in their separate window.',
                            action='store_true')

    # ROI selection
    choices_for_selection_frame = __Strings_or_Positive_Container(['first', 'brightest'])
    parser.add_argument('--selection_frame', help='',
                        choices=choices_for_selection_frame, default='first')
    roiselection = parser.add_mutually_exclusive_group()
    roiselection.add_argument('--ROIs_by_center', help='''Select the ROIs by a single click
                            to set the center. The size of the ROI can be supplied with
                            --ROI_size''',action='store_true')
    roiselection.add_argument('--ROIs_along_line',help='''If given, then ROI selection proceeds like this:
                user selects beginning and end of a line
                ROIs are created along the line in uniform size and distance.
                Size and distance can be supplied with --ROI_size_distance''', action='store_true')
    roiselection.add_argument('--ROIs_on_circle',help='''If given, then ROI selection proceeds like this:
                user selects center and one point on perimeter of circle
                ROIs are created in the center and along the perimeter.
                Size and number or ROIs can be supplied with --ROI_size_num''', action='store_true')
    parser.add_argument('--ROI_size_distance', type=int, help='width, height, distance (all in pixels) of automatically generated ROIs.\
                        Only has effect if --ROIs_along_line is specified.', default=(30, 30, 5), nargs=3)
    parser.add_argument('--ROI_size_num', type=int, help='width (in pixels), height (in pixels),\
                         number of automatically generated ROIs. Only has effect if --ROIs_along_line is specified.',
                         default=(30, 30, 10), nargs=3)
    parser.add_argument('--ROI_size', type=int, help='width, height (in pixels) of  generated ROIs.\
                        Only has effect if --ROIs_by_center is specified.', default=(30, 30), nargs=2)

    #  Tracking algorithm params
    parser.add_argument('--fit_flow_fun',type=str,help='affine or median flow',choices=['median','affine'],
                            default='median')
    parser.add_argument('--skip_frames',type=int,help='number of frames to skip before processing next one',
                            default=0)
    parser.add_argument('--aggfunc', type=str, help='The intensities across a ROI are averages using this average, either the mean or the median.',
...
if __name__=='__main__':
    ...
    if ((args.vis_nir_videos == None) and (args.merged_video == None)):
        ...
        if args.separate_vids:  #  two file dialogues
            vis_path = filedialog.askopenfilename(title='Select visible light video file.')
        ...
        raise RuntimeError(message=
            'Error with the parsing of the video path. You should never reach this part of the source code though.')
    frames_to_process = None  #  We should get away with this
    if args.separate_vids:
        cap1 = setup_input_stream('file', abs_fname=vis_path, transform=frame_and_gray,
                        offset_ms=offset_ms, skip_frames=args.skip_frames,
                        frames_to_process=frames_to_process)
        cap2 = setup_input_stream('file', abs_fname=nir_path, transform=None,
                        offset_ms=offset_ms, skip_frames=args.skip_frames,
                        frames_to_process=frames_to_process)
        next_frame, caps = _next_frame_separate_vids(cap1, cap2)
        vid_folder = os.path.dirname(vis_path)
    else:
        cap1 = setup_input_stream('file', abs_fname=vid_path,
    ...
    '''.format(FPS_reported, FPS, tracking_FPS, args.skip_frames, DT
    ))
    if args.end is None:
        frames_to_process = np.inf  #  We should get away with this
    else:
        frames_to_process = int( 1+ (args.end-args.start)*
                            FPS / (1+args.skip_frames) )
    ...
    if args.store_folder is None:
        ...
        root = tk.Tk()
        root.withdraw()
        store_folder = filedialog.askdirectory(initialdir=os.path.abspath(vid_folder),
                                    title='Select folder to store the generated files in.')
        if not store_folder:    store_folder = os.path.abspath(vid_folder)
    else:
    ...
    if args.selection_frame.lower() == 'brightest':
        ...
        cap_select = setup_input_stream('file', abs_fname=nir_path, transform=None,
            offset_ms=offset_ms, skip_frames=20,
            frames_to_process=min(frames_to_process, 1000))  # BAD: set max frames manually
        max_bright = 0
        ...
        try:
            ...
            cap_select = setup_input_stream('file', abs_fname=nir_path, transform=None,
            offset_ms=offset_select, skip_frames=20,
            frames_to_process=min(frames_to_process, 1000)) # BAD: set max frames manually
    ...
    if args.ROIs_along_line:
        ...
        selected_rois = select_rois_manual(select_frame, display_str="Select the regions of interest.")
        confirm_ROIs_flag = False
    ...
    if confirm_ROIs_flag:
        ...
        for ii, roi in enumerate(selected_rois):
            ...
            cv2h.draw_roi(frame, (roi0[0], roi0[1]), (roi0[0]+roi0[2], roi0[1]+roi0[3] ), label=ii)
        cv2.imshow(select_window_name, cv2h.label_w_background(frame0=frame,
    ...
    if args.aggfunc=='mean':
        ...
        raise RuntimeError('An unknown aggregation function slipped through the parser somehow')
    #%% Set up the TRACKER
    #  create the optical flow object
    if no_tracking_FLAG:
        zeroflow = type(
            'ZeroFlow', (cv2.DenseOpticalFlow,),
            {'calc': lambda self, img0, img1, xxx: np.zeros(img0.shape[:2]+(2,), dtype=np.float)}
            )()  #  an object whose calc() method returns only zeros of the appropriate shape
        tracker = ofrt.optflow_tracker(zeroflow, vis_grey0,
            selected_rois,
            fit_flow_fun='median',
        ...
        tracker = ofrt.optflow_tracker(dis, vis_grey0,
            selected_rois,
    ...
    initial_frame = tracker.overlay_rois(frame=select_frame.copy(), color=(overlay_color,(3,4,250)))
    cv2.imwrite(
        os.path.join(store_folder, args.shortname+'_InitialFrame.png'),
        initial_frame
         )
    ...
    if tracking_vid_FLAG:
        ...
        writer = cv2.VideoWriter( out_file, cv2.VideoWriter_fourcc(*'avc1'),
            max(1,FPS//(tracking_vid_extra_frames_skip+args.skip_frames+1)),
                                tuple(vis0.shape[:2][::-1]), True)
    ...
    else: intensity_fig = intensity_ax = None
    ...
    while True:
        ...
        try:
            ...
            print('[INFO] Acquiring next frame failed, likely the end of the video was reached')
            #  just to be safe:
        ...
        if not (pp % stats_every):
            ...
            print('Processed {:d} frames, {} to go; avg over last {:d} frames: {:.3f} FPS'.
                    format(pp, frames_to_process-pp,  stats_every, stats_every / (t1 - t0))
                )
            t0 = t1
    ...
    if tracking_vid_FLAG:   writer.release()

    if not args.no_plot_stored:
        # move the legend outside
        intensity_ax.legend(loc='upper left', bbox_to_anchor=(1.01,1))
        plt.tight_layout()
        plt.savefig( os.path.join(store_folder, args.shortname+'_Intensities_plot.PNG'))
    ...
    with open(csvfilename, 'w', newline='') as csvfile:
        ...
        # write each row
        for tt, ii in enumerate(zip_longest(*ROI_intensities, fillvalue=args.fill_value)):
            csvwriter.writerow( (tt*DT,) + ii)

    print('[INFO] Wrote {:d} rows and {:d} columns in file {}\n'.format(tt, len(selected_rois), csvfilename))
